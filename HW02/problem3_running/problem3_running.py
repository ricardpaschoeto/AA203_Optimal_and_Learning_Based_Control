...
Original file is located at
    https://colab.research.google.com/drive/191xztGGQLaL14DSVu07jx21lZrWKF8_o
"""
...
def linearize(f, x, u):
    ...
    A, B, c = jax.jacfwd(f, (0))(x, u), jax.jacfwd(f, (1))(x, u), f(x,u)
    ####################################################################################  
    return A, B, c
...
def scp_iteration(f,Q,R,Q_N,s_bar,u_bar,s_star,s0,N,dt,rho,uLB,uUB):
    ...
    for t in range(N):
        ...
        if t > 0:
           ...
           constraints.append(A @ (S[t - 1] - s_bar[t - 1]) + B @ (U[t - 1] - u_bar[t - 1]) + c == S[t])

    S[t + 1] = cvx.Variable(n)
    A, B, c = linearize(f, s_bar[t], u_bar[t])
    constraints.append(A @ (S[t] - s_bar[t]) + B @ (U[t] - u_bar[t]) + c == S[t+1])
