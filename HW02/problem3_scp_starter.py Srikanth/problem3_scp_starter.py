...
def scp(f,Q,R,Q_N,s_star,s0,N,dt,rho,uLB,uUB):
    ...
    s, u = scp_iteration(f_discrete,Q,R,Q_N,s_bar,u_bar,s_star,s0,N,dt,rho,uLB,uUB)
...
def scp_iteration(f,Q,R,Q_N,s_bar,u_bar,s_star,s0,N,dt,rho,uLB,uUB):
    ...
    for t in range(N):
        ...
        cost_terms.append(cvx.quad_form(S[t] - s_star, Q)) # State cost
        cost_terms.append(cvx.quad_form(U[t] - u_bar[t], R)) # Control cost
