# -*- coding: utf-8 -*-
"""problem3_scp_starter.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1r37fpYVxUlV9qI35EtFCIs270bvYCJ8h
"""
...
def linearize(f, x, u):
    ...
    A, B, c = jax.jacfwd(f, (0,1))(x,u), f(x, u) 
    ###########################################################################    
    return A, B
...
def scp_iteration(f,Q,R,Q_N,s_bar,u_bar,s_star,s0,N,dt,rho,uLB,uUB):
    ...
    for ii in range(N + 1):
        ...
        U[ii] = cvx.Variable(m)
        if ii == 0: # s[0] s0
            constraints.append(S[ii] == s0)
        if ii <= N:
            if ii > 0: # dynamics: ii = 1, ..., N
                constraints.append(A[ii-1]@(S[ii-1] - s_bar[ii-1]) + B[ii-1]@(U[ii-1] - u_bar[ii-1]) + c[ii-1] == S[ii])
            constraints.append(U[ii] <= uUB)
            constraints.append(U[ii] >= uLB)
            if ii != N: # U E ii = 0, ..., N - 1 
                constraints.append(cvx.norm(U[ii] - u_bar[i], "inf") <= rho)
            constraints.append(cvx.norm(S[ii] - s_bar[i], "inf") <= rho)
            cost_terms.append(cvx.quad_form(U[ii], R))
            cost_terms.append(cvx.quad_form(U[ii], R))
        if ii == N: # last cost in sN
            cost_terms.append(cvx.quad_form(S[ii] - s_star, Q_N)

    obj = cvx.Minimize(dt * cvx.sum(cost_terms))
    problem = cvx.Problem(obj, constraints)
    problem.solve()

    u = [u for u.value in U]
    s = [s for s.value in S]
    ###########################################################################
    return s,u
