...
def linearize(f, s, u):
    ...
    A, B = jax.jacfwd(lambda s:f(s, jnp.ones(u.shape)))(s), jax.jacfwd(lambda u:f(jnp.ones(s.shape), u))(u) 
    ###########################################################################
    return A, B
...
def ilqr(f, s0, s_goal, N, Q, R, Qf):
    ...
    def costs():
        ...
        for k in range(N-1):
            qkT = np.dot((s[k] - s_goal).T,Q)
            rkT = np.dot(u[k],R)
			# c(sk, uk) = (1/2)*(sk.T*Qk*sk) + (1/2)*(uk.T*Rk*uk) + (1/2)*(dsk.T*Qk*dsk) + (1/2)*(duk.T*Rk*duk) + qk.T*dsk + rk.T*duk + s_goal.T*Qk*s_goal
            c[k] = 0.5*np.dot(s[k].T, np.dot(Q, s[k])) + 0.5*np.dot(u[k].T, np.dot(R, u[k])) + 0.5*np.dot((s[k] - s_bar[k]).T, np.dot(Q, (s[k] - s_bar[k]))) \
            + 0.5*np.dot((u[k]-u_bar[k]).T, np.dot(R, (u[k]-u_bar[k]))) + np.dot(qkT, (s[k] - s_bar[k])) + np.dot(rkT, (u[k] - u_bar[k])) + np.dot(s_goal.T, np.dot(Q,s_goal))
            cs[k] = qkT # qk.T
            cu[k] = rkT # rk.T
            cuu[k] = np.array(R) # just R
            css[k] = np.array(Q) # just Q
        
        # cost for c ??? (1/2)*sN.T*QN*sN + (1/2)*dsN.T*QN*dsN + qN.T*dsN + s_goal.T*QN*s_goal
        qNT = np.dot((s[-1] - s_goal).T,Qf)
        c[-1] = 0.5*np.dot(s[-1].T, np.dot(Qf, s[-1])) + 0.5*np.dot((s[-1] - s_bar[-1]).T, np.dot(Qf, (s[-1] - s_bar[-1]))) + np.dot(qNT, (s[-1] - s_bar[-1])) \
        + np.dot(s_goal.T, np.dot(Qf, s_goal))
        cs[-1] = qNT # qN.T ????
        css[-1] = Qf # final cost
    ...
    for _ in range(max_iters):
        ...
        for k in range(N - 1, -1, -1):
            Qk = c[k] + v
            Qs = cs[k] + np.dot(A[k].T, v_bold)
            Qu = cu[k] + np.dot(B[k].T, v_bold)
            Qss = css[k] + np.dot(A[k].T, np.dot(V,A[k]))
            Quu = cuu[k] + np.dot(B[k].T, np.dot(V,B[k]))
            Qus = cus[k] + np.dot(B[k].T, np.dot(V,A[k]))

            l[k] = -np.dot(np.linalg.inv(Quu), Qu)
            L[k] = -np.dot(np.linalg.inv(Quu), Qus)

            v = Qk - 0.5*np.dot(l[k].T, np.dot(Quu,l[k]))
            v_bold = Qs - np.dot(L[k].T,np.dot(Quu,l[k]))
            V = Qss - np.dot(L[k].T, np.dot(Quu,L[k]))

        for k in range(N - 1):
            u[k] += l[k] + np.dot(L[k], s[k] - s_bar[k]) 
        #######################################################################
